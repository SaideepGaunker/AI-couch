...
"""
    ...
    def generate_questions(
        ...
        # Check Gemini API availability - use fallback if not available
        if not self.model or not settings.GEMINI_API_KEY or settings.GEMINI_API_KEY == "your-gemini-api-key-here":
            logger.warning("Gemini API not configured properly, using fallback questions")
            return self.fallback_service.get_questions_by_role(role, difficulty, count)
        ...
        try:
            ...
            if question_type == "technical" and context and self._get_role_attribute(context, 'main_role'):
            ...
            if questions_data and len(questions_data) > 0:
                ...
                if questions_data:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to store questions in database: {str(store_error)}")
                        # Continue anyway - questions are still generated and can be used
                    
                    return questions_data
            else:
                logger.warning("Gemini API returned empty questions, using fallback")
                return self.fallback_service.get_questions_by_role(role, difficulty, count)
            ...
            # Always use fallback questions when Gemini fails
            logger.warning("Using fallback questions due to Gemini API error")
            return self.fallback_service.get_questions_by_role(role, difficulty, count)
    ...
    def evaluate_answer(
        ...
        """Evaluate user's answer using Gemini API with quality pre-checks"""
        
        try:
            # Pre-process answer to detect obvious quality issues
            quality_check = self._pre_check_answer_quality(answer, question)
            if quality_check["is_poor_quality"]:
                logger.warning(f"Answer failed quality pre-check: {quality_check['reason']}")
                return {
                    "overall_score": quality_check["suggested_score"],
                    "content_quality": quality_check["suggested_score"],
                    "communication": max(10, quality_check["suggested_score"] - 5),
                    "technical_accuracy": quality_check["suggested_score"],
                    "feedback": f"Answer quality issue detected: {quality_check['reason']}",
                    "suggestions": [
                        "Please provide a more detailed and relevant response",
                        "Ensure your answer directly addresses the question",
                        "Use proper language and avoid nonsensical content"
                    ],
                    "quality_flags": quality_check["flags"],
            ...
            # Post-process evaluation to ensure realistic scoring
            evaluation = self._post_process_evaluation(evaluation, answer, question)
    ...
    def _build_contextual_question_prompt(
        ...
        validation_criteria = self._get_role_validation_criteria(self._get_role_attribute(context, 'main_role', role) if context else role)
    ...
    def _build_evaluation_prompt(
        ...
        """Build prompt for answer evaluation with strict quality assessment"""
        ...
        prompt = f"""
        You are a strict interview evaluator. Evaluate this interview answer with high standards.
        ...
        1. Content Quality (0-100):
           - 0-20: Nonsensical, irrelevant, or no meaningful content
           - 21-40: Minimal relevance, very basic or incorrect information
           - 41-60: Some relevant content but lacks depth or has inaccuracies
           - 61-80: Good content with minor gaps or improvements needed
           - 81-100: Excellent, comprehensive, accurate content
        
        2. Communication (0-100):
           - 0-20: Incoherent, unprofessional, or unintelligible
           - 21-40: Poor structure, unclear communication
           - 41-60: Basic communication with some clarity issues
           - 61-80: Clear communication with minor issues
           - 81-100: Excellent, professional, well-structured communication
        
        3. Relevance (0-100):
           - 0-20: Completely off-topic or doesn't address the question
           - 21-40: Tangentially related but misses the main point
           - 41-60: Partially addresses the question
           - 61-80: Addresses most aspects of the question
           - 81-100: Directly and completely addresses the question

        SPECIAL DETECTION RULES:
        - If answer contains gibberish, random characters, or is clearly nonsensical: Score 0-15
        - If answer is extremely short (under 10 words) and doesn't address question: Score 0-25
        - If answer is completely off-topic: Score 0-30
        - If answer shows no understanding of the question: Score 0-35

        Calculate overall_score as the average of the three criteria scores.
        ...
        {{
            ...
            "strengths": ["Any positive aspects found"],
            "improvements": ["Specific areas needing improvement"],
            "suggestions": ["Actionable advice for better answers"],
            "quality_flags": ["poor_content", "off_topic", "unclear"] // Add flags for detected issues
        }}
        """
        
        return prompt
    
    def _pre_check_answer_quality(self, answer: str, question: str) -> Dict[str, Any]:
        """Pre-check answer quality to catch obvious issues before AI evaluation"""
        ...
        if len(answer_clean) < 5:
            return {
                ...
                "reason": "Answer is too short or empty",
                "suggested_score": 5,
                "flags": ["too_short"]
            }
        
        # Check for nonsensical content (repeated characters, gibberish)
        import re
        
        # Check for excessive repeated characters
        if re.search(r'(.)\1{10,}', answer_clean):  # Same character repeated 10+ times
            return {
                "is_poor_quality": True,
                "reason": "Answer contains excessive repeated characters",
                "suggested_score": 8,
                "flags": ["gibberish", "repeated_chars"]
            }
        
        # Check for random character sequences
        random_patterns = [
            r'[a-z]{1,2}[0-9]{3,}',  # Random letters followed by numbers
            r'[qwerty]{5,}',  # Keyboard mashing
            r'[asdf]{5,}',    # More keyboard mashing
            r'[zxcv]{5,}',    # Even more keyboard mashing
        ]
        ...
        for pattern in random_patterns:
            if re.search(pattern, answer_clean):
                return {
                    "is_poor_quality": True,
                    "reason": "Answer appears to contain random or nonsensical text",
                    "suggested_score": 10,
                    "flags": ["gibberish", "random_text"]
                }
        
        # Check for completely off-topic answers (very basic check)
        # If answer is very short and contains no words from the question
        if len(answer_clean.split()) < 10:
            question_words = set(re.findall(r'\b\w{3,}\b', question_clean))
            answer_words = set(re.findall(r'\b\w{3,}\b', answer_clean))
            
            # Remove common words
            common_words = {'the', 'and', 'for', 'are', 'but', 'not', 'you', 'all', 'can', 'had', 'her', 'was', 'one', 'our', 'out', 'day', 'get', 'has', 'him', 'his', 'how', 'its', 'may', 'new', 'now', 'old', 'see', 'two', 'who', 'boy', 'did', 'she', 'use', 'her', 'way', 'many', 'then', 'them', 'well', 'were'}
            question_words -= common_words
            answer_words -= common_words
            
            if question_words and not (question_words & answer_words):
                return {
                    "is_poor_quality": True,
                    "reason": "Answer appears completely unrelated to the question",
                    "suggested_score": 15,
                    "flags": ["off_topic", "no_relevance"]
                }
        
        # Check for single word answers to complex questions
        if len(answer_clean.split()) == 1 and len(question_clean.split()) > 5:
            return {
                "is_poor_quality": True,
                "reason": "Single word answer to a complex question",
                "suggested_score": 20,
                "flags": ["too_brief", "insufficient_detail"]
            }
        
        return {
            "is_poor_quality": False,
            "reason": "Answer passed basic quality checks",
            "suggested_score": None,
            "flags": []
        }
    
    def _post_process_evaluation(self, evaluation: Dict[str, Any], answer: str, question: str) -> Dict[str, Any]:
        """Post-process evaluation to ensure realistic and consistent scoring"""
        
        try:
            # Ensure all required fields exist
            if "overall_score" not in evaluation:
                evaluation["overall_score"] = 35
            
            if "scores" not in evaluation:
                evaluation["scores"] = {
                    "content_quality": evaluation.get("content_quality", 35),
                    "communication": evaluation.get("communication", 35),
                    "relevance": evaluation.get("relevance", 35)
                }
            
            # Validate score ranges (0-100)
            evaluation["overall_score"] = max(0, min(100, evaluation["overall_score"]))
            
            for key in evaluation["scores"]:
                evaluation["scores"][key] = max(0, min(100, evaluation["scores"][key]))
            
            # Recalculate overall score from component scores for consistency
            component_scores = list(evaluation["scores"].values())
            if component_scores:
                calculated_overall = sum(component_scores) / len(component_scores)
                # Use the lower of calculated vs reported overall score to prevent inflation
                evaluation["overall_score"] = min(evaluation["overall_score"], calculated_overall)
            
            # Apply additional penalties for very short answers
            answer_length = len(answer.strip().split())
            if answer_length < 5:
                penalty_factor = 0.5  # 50% penalty for very short answers
                evaluation["overall_score"] *= penalty_factor
                for key in evaluation["scores"]:
                    evaluation["scores"][key] *= penalty_factor
                
                if "quality_flags" not in evaluation:
                    evaluation["quality_flags"] = []
                evaluation["quality_flags"].append("length_penalty_applied")
            
            # Ensure minimum standards - if all scores are suspiciously high, apply reality check
            if evaluation["overall_score"] > 80 and answer_length < 20:
                logger.warning(f"Suspiciously high score ({evaluation['overall_score']}) for short answer ({answer_length} words)")
                evaluation["overall_score"] = min(evaluation["overall_score"], 60)
                for key in evaluation["scores"]:
                    evaluation["scores"][key] = min(evaluation["scores"][key], 65)
            
            # Round scores to 1 decimal place
            evaluation["overall_score"] = round(evaluation["overall_score"], 1)
            for key in evaluation["scores"]:
                evaluation["scores"][key] = round(evaluation["scores"][key], 1)
            
            return evaluation
            
        except Exception as e:
            logger.error(f"Error in post-processing evaluation: {str(e)}")
            return evaluation
    ...
    def _validate_question_relevance(self, questions: List[Dict[str, Any]], role_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        sub_role = self._get_role_attribute(role_context, 'sub_role', '')
        specialization = self._get_role_attribute(role_context, 'specialization', '')
        tech_stacks = self._get_role_attribute(role_context, 'tech_stacks', [])
        question_tags = self._get_role_attribute(role_context, 'question_tags', [])
    ...
    def _validate_contextual_relevance(
        ...
        main_role = self._get_role_attribute(role_context, 'main_role', '') if role_context else ''
...
"""
    ...
    def _get_fallback_evaluation(self) -> Dict[str, Any]:
        """Provide fallback evaluation when AI evaluation fails - now with realistic scoring"""
        return {
            "overall_score": 35,  # Lower default score to reflect lack of proper evaluation
            "content_quality": 35,
            "communication": 35,
            "technical_accuracy": 35,
            "feedback": "Unable to properly evaluate your response due to technical limitations. This may affect your difficulty progression.",
            "suggestions": [
                "Please try again with a more detailed response",
                "Ensure your answer directly addresses the question asked",
                "Provide specific examples and clear explanations"
            ],
            "fallback_used": True,
            "warning": "This score may not reflect actual performance due to evaluation system limitations"
        }
